...
基于 **Vue Vben Admin v5.5.9** 独立拆分而来的 Naive UI 版本单页应用（SPA）。

## 📖 项目简介

`vue-vben-admin-naive` 是从原始 [Vue Vben Admin](https://github.com/vbenjs/vue-vben-admin) monorepo 架构中独立拆分出来的 **Naive UI** 版本。本项目保留了 Vue Vben Admin 的所有核心功能特性，但完全脱离 pnpm workspace 和 monorepo 依赖，可作为独立项目单独部署、测试和发布。
...
- 🎨 **基于 Naive UI 2.x** - 使用 Naive UI 作为 UI 组件库
...
- **Naive UI**: [Naive UI 文档](https://www.naiveui.com)
